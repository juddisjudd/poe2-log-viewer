...
function App() {
  ...
  return (
    <div className="h-screen bg-poe-black text-gray-100 flex flex-col overflow-hidden">
      {/* Custom Titlebar */}
      <div 
        className="bg-poe-darker border-b border-poe-border px-3 py-1.5 flex items-center justify-between shrink-0 select-none"
      ...
      >
        <div className="flex items-center gap-3">
          ...
          {updateInfo.available && (
            <button
              ...
              className="bg-poe-forest hover:bg-poe-moss/50 text-poe-gold px-2 py-0.5 rounded-sm text-xs font-medium transition-colors animate-pulse border border-poe-moss/50"
          ...
          {isCheckingUpdate ? (
            <span className="text-xs text-poe-gold-dim animate-pulse">
              Checking...
            </span>
          ) : (
            <button
              onClick={() => checkForUpdates()}
              className="text-poe-text-muted hover:text-poe-gold transition-all duration-200 text-xs"
        ...
        <button
          onClick={handleClose}
          className="text-poe-text-muted hover:text-red-400 hover:bg-poe-blood/50 px-2 py-0.5 rounded-sm transition-colors text-sm font-medium"
          title="Close"
        >
          ✕
        </button>
      ...
      <div className="bg-poe-dark border-b border-poe-border p-3 shrink-0">
        <div className="flex items-center justify-between gap-3">
          {/* File info */}
          <div className="text-sm text-poe-text-dim">
            {currentFile ? (
              <span>
                <span className="text-poe-text-muted">File:</span>
                <span className="text-gray-300 ml-1" title={currentFile}>
                  {getShortPath(currentFile)}
                </span>
              </span>
            ) : (
              <span className="text-poe-text-muted">No file selected</span>
            )}
          </div>
          ...
          <div className="flex items-center gap-3">
            <div className="text-sm text-poe-text-dim">
              <span className="text-poe-text-muted">Logs:</span>
            ...
            <button
              onClick={pickFile}
              className="bg-poe-muted hover:bg-poe-border text-poe-gold px-3 py-1.5 rounded-sm text-sm font-medium transition-colors border border-poe-border"
            >
              Select File
            </button>
          </div>
        ...
        <div className="flex items-center gap-4 mt-2 text-xs">
          <label className="flex items-center gap-2 text-poe-text-muted hover:text-poe-gold cursor-pointer transition-colors">
            <input
              type="checkbox"
              ...
              className="w-3 h-3 rounded-sm border-poe-border bg-poe-dark text-poe-gold focus:ring-poe-gold focus:ring-1 accent-poe-gold"
            />
        ...
        {error && (
          <div className="mt-2 text-xs text-red-300 bg-poe-blood/30 border border-poe-crimson p-2 rounded-sm">
            {error}
          </div>
        )}
      </div>
      ...
      {/* Filter Panel */}
      <div className="bg-poe-dark border-b border-poe-border shrink-0">
        <FilterPanel
          filters={filters}
